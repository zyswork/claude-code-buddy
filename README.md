...
| 值 | 效果 |
|---|---|
| `both`（默认）| 单行 `★ 🐰 (·..·) 小墨 Lv5 🙂 "又熬夜？"` |
| `emoji` | 单行 `★ 🐰 小墨 Lv5 🙂` |
| `ascii` | 单行 `★ (·..·) 小墨 Lv5 🙂` |
| `sprite` | **多行** — 最接近原版 `/buddy`：sprite + 名字 + 气泡 |

**sprite 模式效果**（需要你的 Claude Code 支持多行 statusline）：
...
先用默认 `both`；想要多行的在 `~/.zshrc` 加：
```bash
export BUDDY_STATUSLINE_STYLE=sprite
```
重启 Claude Code 看看能不能完整渲染，不能就回 `both`。
